...
?>
	...
	<?php if ( have_comments() ) : ?>
		...
		<div id="comments-title">
			<h3>
				<?php
					printf( _n( '<span>%1$s</span> Comments', '<span>%1$s</span> Comments', get_comments_number(), 'sento' ),
	...
	<?php 
		...
		$comments_args = array(
			...
			'comment_field' =>  
				...
				'<textarea id="comment" name="comment" placeholder="' . esc_attr__( 'Your Message', 'sento' ) . '" cols="45" rows="8" aria-required="true">' .
				'</textarea></p>',
			'fields' => apply_filters( 'comment_form_default_fields', array (
				'author' =>
					'<p class="comment-form-author one_third">' .
					'<input id="author" name="author" placeholder="' . esc_attr__( 'Your Name (Required)', 'sento' ) . '" type="text" value="' . esc_attr( $commenter['comment_author'] ) .
					'" size="30" /></p>',
				'email' =>
					'<p class="comment-form-email one_third">' .
					'<input id="email" name="email" placeholder="' . esc_attr__( 'Your Email (Required)', 'sento' ) . '" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) .
					'" size="30" /></p>',
				'url' =>
					'<p class="comment-form-url one_third last">' .
					'<input id="url" name="url" placeholder="' . esc_attr__( 'Your Website', 'sento' ). '" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) .
					'" size="30" /></p>'
			) ),
		);
