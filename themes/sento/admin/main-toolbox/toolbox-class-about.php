...
if ( ! class_exists( 'thinkup_toolbox_about_page' ) ) {
	...
	class thinkup_toolbox_about_page {
		...
		public function setup_config() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : esc_html__( 'About', 'sento' ) . ' ' . $this->theme_name;
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : esc_html__( 'About', 'sento' ) . ' ' . $this->theme_name;
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( __( 'Welcome! Thank you for choosing %1$s! To get started please make sure you visit our %2$s<strong>welcome page</strong>%3$s.', 'sento' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=thinkup-welcome' ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=thinkup-welcome' ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( __( 'Get started with %s', 'sento' ), $this->theme_name ) . '</a></p>' );
		...
		public function welcome_admin_notice() {
			...
			if ( ! empty( $this->notification ) ) {
				...
				if ( current_user_can( 'edit_theme_options' ) && ! get_option( 'thinkup_notice_welcome' ) ) {
					...
					echo '<span class="screen-reader-text">' . esc_html__('Dismiss this notice.', 'sento'). '</span>';
					echo '</a>';
		...
		public function hide_notice() {
			if (isset($_GET['thinkup-hide-notice']) && isset($_GET['_thinkup_notice_nonce'])) {
				if (!wp_verify_nonce($_GET['_thinkup_notice_nonce'], 'thinkup_hide_notices_nonce')) {
					wp_die(esc_html__('Action failed. Please refresh the page and retry.', 'sento'));
				}
				if (!current_user_can('edit_theme_options')) {
					wp_die(esc_html__('You do not have the necessary permission to perform this action.', 'sento'));
				}
